...
from data import create_dataloader, convert_example, convert_example_with_attention_token
from model import QuestionMatchingAttention, QuestionMatchingAttentionWithBiLSTM
from train import do_train
import json
from data import RawData,QMSet
...
if __name__ == "__main__":
    ...
    # tokenizer = AutoTokenizer.from_pretrained("ernie-3.0-xbase-zh")
    pretrained_model = ppnlp.transformers.ErnieGramModel.from_pretrained(
        model_config.init_ckpt)
    # pretrained_model =ppnlp.transformers.ErnieModel.from_pretrained("ernie-3.0-xbase-zh")
    ...
    def batchify_fn(samples):
        ...
        keys = ['input_ids', 'token_type_ids', 'select_tokens','label']
    ...
    model = QuestionMatchingAttentionWithBiLSTM(pretrained_model, model_config)
    do_train(model,train_config,train_data_loader = train_data_loader,dev_data_loader=None,fold_num='single',attack=train_config.attack)
