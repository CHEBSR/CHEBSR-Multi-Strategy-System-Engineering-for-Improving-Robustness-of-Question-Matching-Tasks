...
# from paddlenlp.modules import BasicModule

#add215
class QuestionMatchingAttentionWithBiLSTM(BasicModule):
    def __init__(self, pretrained_model, config):
        super(QuestionMatchingAttentionWithBiLSTM, self).__init__()
        self.ptm = pretrained_model
        self.config = config
        self.dropout = nn.Dropout(config.dropout if config.dropout is not None else 0.15)

        # num_labels = 2 (similar or dissimilar)
        self.classifier = nn.Linear(self.ptm.config["hidden_size"]*2, 2) # 由于加入了BiLSTM，所以需要将分类器的输入维度扩大一倍
        self.bilstm = nn.LSTM(self.ptm.config["hidden_size"], self.ptm.config["hidden_size"], num_layers=1, direction='bidirectional')
        self.rdrop_coef = config.rdrop_coef
        self.rdrop_loss = ppnlp.losses.RDropLoss()
    ...
    def forward(self,
        ...
        select_length = select_tokens.sum(axis=1)[:, 0].detach()  # 每个样本的mask后的长度
        x = hidden_state * select_tokens.detach()  #
        x, _ = self.bilstm(x) # 对选择的词汇进行BiLSTM操作
        x = x.sum(axis=1) / (select_length.reshape((batch_shape, 1))) # 将BiLSTM输出的每个词汇向量加和，然后除以有效长度，得到句子的表示
...
#add216
class QuestionMatchingAttentionDomainWithBiLSTM(BasicModule):
    def __init__(self, pretrained_model, config):
        super(QuestionMatchingAttentionDomainWithBiLSTM, self).__init__()
        self.ptm = pretrained_model
        self.config = config
        self.dropout = nn.Dropout(config.dropout if config.dropout is not None else 0.15)

        self.bilstm = nn.LSTM(input_size=self.ptm.config["hidden_size"], 
                              hidden_size=self.ptm.config["hidden_size"]//2,
                              num_layers=1,
    ...
    def forward(self,
        ...
        # Add BiLSTM layer
        lstm_out, _ = self.bilstm(hidden_state)

        select_length = select_tokens.sum(axis=1)[:, 0].detach()  # 每个样本的mask后的长度
        x = lstm_out * select_tokens.detach()  #
